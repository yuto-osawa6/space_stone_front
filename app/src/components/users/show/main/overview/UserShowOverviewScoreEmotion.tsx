...
export const UserShowOverviewScoreEmotion:React.FC<Props> = function UserShowOverviewScoreEmotionFunc(Props){
  ...
  useEffect(()=>{
    const averageScore = Math.floor((Props.emotionCount/Props.user.scoreEmotionAllCount)*100)
    if(averageScore<=10){
  ...
  return(
    <>
      ...
      >{Props.emotion.emotion} ({((Props.emotionCount/Props.user.scoreEmotionAllCount)*100).toFixed(1)}%)</li>
    </>
  )
}