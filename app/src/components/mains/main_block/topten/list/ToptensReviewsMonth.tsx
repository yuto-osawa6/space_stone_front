...
import { execToptenLikeAll, execToptenReviewMonth } from "@/lib/api/mains/toptens";
...
export const ToptensReviewsMonth:React.FC = function ToptensReviewsMonth(){
  ...
  const setupHandler = async() => {
    const res = await execToptenReviewMonth()
    if(res.status === 200){
      if(isMounted){
